...
<div class="gradient-ws4u text-white py-24 px-4">
    <div class="max-w-5xl mx-auto">
        <a href="{{ route('products') }}" class="text-sky-300 hover:text-white text-sm mb-6 inline-flex items-center gap-1">← {{ app()->getLocale() == 'cs' ? 'Zpět na produkty' : 'Back to products' }}</a>
        <div class="flex items-center gap-5 mb-8">
            ...
            <div>
                ...
                <p class="text-sky-300 text-lg mt-1">{{ app()->getLocale() == 'cs' ? 'Software přesně pro vaše potřeby' : 'Software tailored to your needs' }}</p>
            </div>
        </div>
...
<section class="py-20 bg-white">
    <div class="max-w-6xl mx-auto px-4">
        <h2 class="text-3xl font-bold text-slate-900 text-center mb-12">{{ app()->getLocale() == 'cs' ? 'Náš vývojový proces' : 'Our Development Process' }}</h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
                <h2 class="text-3xl font-bold text-slate-900 mb-6">{{ app()->getLocale() == 'cs' ? 'Co umíme postavit' : 'What We Can Build' }}</h2>
                <div class="space-y-3">
                    @php
            ...
            <div>
                <h2 class="text-3xl font-bold text-slate-900 mb-6">{{ app()->getLocale() == 'cs' ? 'Technologie' : 'Technologies' }}</h2>
                ...
                <div class="bg-sky-600 text-white rounded-2xl p-6 shadow-xl">
                    <h3 class="font-bold text-lg mb-2">{{ app()->getLocale() == 'cs' ? 'Bezplatná konzultace' : 'Free Consultation' }}</h3>
                    <p class="text-sky-100 text-sm mb-4">{{ app()->getLocale() == 'cs' ? 'Řekneme vám upřímně, zda a jak váš projekt zvládneme. Bez závazků, zdarma.' : 'We will tell you honestly if and how we can handle your project. No commitment, for free.' }}</p>
                    <a href="{{ route('contact') }}" class="block text-center bg-white text-sky-700 hover:bg-sky-50 py-3 rounded-xl font-semibold transition-all">
