...
<div class="gradient-ws4u text-white py-24 px-4">
    <div class="max-w-5xl mx-auto">
        <a href="{{ route('products') }}" class="text-orange-300 hover:text-white text-sm mb-6 inline-flex items-center gap-1">← {{ app()->getLocale() == 'cs' ? 'Zpět na produkty' : 'Back to products' }}</a>
        <div class="flex items-center gap-5 mb-8">
            ...
            <div>
                ...
                <p class="text-orange-300 text-lg mt-1">{{ app()->getLocale() == 'cs' ? 'Kompletní POS systém pro gastronomii' : 'Complete POS system for gastronomy' }}</p>
            </div>
        </div>
...
<section class="py-20 bg-white">
    <div class="max-w-6xl mx-auto px-4">
        <h2 class="text-3xl font-bold text-slate-900 text-center mb-12">{{ app()->getLocale() == 'cs' ? 'Moduly GastroTech' : 'GastroTech Modules' }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
            ...
            <div class="bg-slate-50 rounded-2xl p-6 border border-slate-100 card-hover">
        ...
        <!-- Pricing -->
        <div class="mb-20">
            <h2 class="text-3xl font-bold text-slate-900 text-center mb-12">{{ app()->getLocale() == 'cs' ? 'Cenové plány' : 'Pricing Plans' }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-8 flex flex-col shadow-sm">
                    ...
                    <p class="text-slate-500 text-sm mb-6">{{ app()->getLocale() == 'cs' ? 'Pro malé kavárny a bistra' : 'For small cafes and bistros' }}</p>
                    <div class="mb-6">
                    ...
                    <ul class="space-y-4 mb-8 flex-grow text-sm text-slate-600">
                        ...
                        <li class="flex items-center gap-2 text-slate-300">❌ {{ app()->getLocale() == 'cs' ? 'Personální modul' : 'Staff module' }}</li>
                        <li class="flex items-center gap-2 text-slate-300">❌ HACCP</li>
                    </ul>
                    <a href="{{ route('contact', ['interest' => 'GastroTech Basic']) }}" class="block text-center py-3 rounded-xl border-2 border-orange-500 text-orange-600 font-bold hover:bg-orange-50 transition-colors">{{ app()->getLocale() == 'cs' ? 'Zkusit Basic' : 'Try Basic' }}</a>
                </div>
                <!-- Standard -->
                <div class="bg-slate-900 rounded-2xl border-2 border-orange-500 p-8 flex flex-col shadow-xl scale-105 z-10 relative text-white">
                    <div class="absolute top-0 right-8 -translate-y-1/2 bg-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">{{ app()->getLocale() == 'cs' ? 'Nejoblíbenější' : 'Most Popular' }}</div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-8 flex flex-col shadow-sm">
                    ...
                    <p class="text-slate-500 text-sm mb-6">{{ app()->getLocale() == 'cs' ? 'Pro velké provozy a sítě' : 'For large operations and networks' }}</p>
                    <div class="mb-6">
                    ...
                    <a href="{{ route('contact', ['interest' => 'GastroTech Premium']) }}" class="block text-center py-3 rounded-xl border-2 border-slate-800 text-slate-800 font-bold hover:bg-slate-50 transition-colors">{{ app()->getLocale() == 'cs' ? 'Kontaktovat' : 'Contact' }}</a>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-6 mb-6">
                        <div class="bg-slate-800 rounded-xl p-4 text-center"><p class="text-3xl font-black text-orange-400">-15h</p><p class="text-slate-400 text-xs mt-1">{{ app()->getLocale() == 'cs' ? 'Ušetřených hodin týdně' : 'Hours saved weekly' }}</p></div>
                        <div class="bg-slate-800 rounded-xl p-4 text-center"><p class="text-3xl font-black text-green-400">+12%</p><p class="text-slate-400 text-xs mt-1">{{ app()->getLocale() == 'cs' ? 'Nárůst tržeb' : 'Revenue increase' }}</p></div>
                        <div class="bg-slate-800 rounded-xl p-4 text-center"><p class="text-3xl font-black text-sky-400">0</p><p class="text-slate-400 text-xs mt-1">{{ app()->getLocale() == 'cs' ? 'HACCP pokut' : 'HACCP fines' }}</p></div>
                        <div class="bg-slate-800 rounded-xl p-4 text-center"><p class="text-3xl font-black text-pink-400">4.9★</p><p class="text-slate-400 text-xs mt-1">{{ app()->getLocale() == 'cs' ? 'Spokojenost klientů' : 'Client satisfaction' }}</p></div>
                    </div>
                </div>
