...
@section('content')
<div class="bg-gradient-to-br from-purple-950 via-[#1e293b] to-[#0f172a] text-white py-24 px-4 border-b border-slate-800">
    <div class="max-w-5xl mx-auto">
        <a href="{{ route('products') }}" class="text-purple-400 hover:text-white text-sm mb-6 inline-flex items-center gap-1 font-mono">← {{ app()->getLocale() == 'cs' ? 'Zpět na produkty' : 'Back to products' }}</a>
        <div class="flex items-center gap-5 mb-8">
            ...
            <div>
                ...
                <p class="text-purple-400 text-lg mt-1">{{ app()->getLocale() == 'cs' ? 'Pro organizátory festivalů a kulturních akcí' : 'For organizers of festivals and cultural events' }}</p>
            </div>
        </div>
...
<section class="py-20 bg-[#111827]">
    <div class="max-w-6xl mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
            <div class="bg-[#1e293b]/50 rounded-2xl p-8 border border-slate-800 card-hover">
                <div class="text-4xl mb-4">🗓️</div>
                <h3 class="font-bold text-white text-xl mb-3">{{ app()->getLocale() == 'cs' ? 'Program a otevírací doby' : 'Program and Opening Hours' }}</h3>
                <p class="text-slate-400">{{ app()->getLocale() == 'cs' ? 'Snadno spravujte program akce, otevírací doby areálů a scén. Návštěvníci vidí vše aktuálně v mobilu.' : 'Easily manage the event program, opening hours of areas and stages. Visitors see everything current on their mobile.' }}</p>
            </div>
            <div class="bg-[#1e293b]/50 rounded-2xl p-8 border border-slate-800 card-hover">
                <div class="text-4xl mb-4">📱</div>
                <h3 class="font-bold text-white text-xl mb-3">{{ app()->getLocale() == 'cs' ? 'Registrace a vstupenky' : 'Registration and Tickets' }}</h3>
                <p class="text-slate-400">{{ app()->getLocale() == 'cs' ? 'Registrace účastníků s QR kódy pro vstup. Správa kapacit a prodej vstupenek online.' : 'Registration of participants with QR codes for entry. Capacity management and online ticket sales.' }}</p>
            </div>
            <div class="bg-[#1e293b]/50 rounded-2xl p-8 border border-slate-800 card-hover">
                <div class="text-4xl mb-4">👥</div>
                <h3 class="font-bold text-white text-xl mb-3">{{ app()->getLocale() == 'cs' ? 'Počítání návštěvníků' : 'Visitor Counting' }}</h3>
                <p class="text-slate-400">{{ app()->getLocale() == 'cs' ? 'Real-time sledování obsazenosti zón a celého areálu. Okamžitý přehled pro pořadatele i bezpečnostní složky.' : 'Real-time tracking of occupancy of zones and the entire area. Instant overview for organizers and security forces.' }}</p>
            </div>
            <div class="bg-[#1e293b]/50 rounded-2xl p-8 border border-slate-800 card-hover">
                <div class="text-4xl mb-4">📢</div>
                <h3 class="font-bold text-white text-xl mb-3">{{ app()->getLocale() == 'cs' ? 'Komunikace s návštěvníky' : 'Communication with Visitors' }}</h3>
                <p class="text-slate-400">{{ app()->getLocale() == 'cs' ? 'Push notifikace, změny programu v reálném čase, bezpečnostní upozornění. Vždy v kontaktu s publikem.' : 'Push notifications, real-time program changes, security alerts. Always in touch with the audience.' }}</p>
            </div>
        </div>

        <!-- Pricing -->
        <div class="mb-20">
            <h2 class="text-3xl font-bold text-white text-center mb-12">{{ app()->getLocale() == 'cs' ? 'Ceník pro organizátory' : 'Pricing for Organizers' }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <!-- Single Event -->
                <div class="bg-[#1e293b]/50 rounded-2xl border border-slate-800 p-8 flex flex-col shadow-sm">
                    <h3 class="text-xl font-bold text-white mb-2">Single Event</h3>
                    <p class="text-slate-400 text-sm mb-6">{{ app()->getLocale() == 'cs' ? 'Jednorázová akce do 1 000 lidí' : 'One-time event up to 1,000 people' }}</p>
                    <div class="mb-6">
                        <span class="text-4xl font-black text-white">4 900 Kč</span>
                    ...
                    <ul class="space-y-4 mb-8 flex-grow text-sm text-slate-300">
                        ...
                        <li class="flex items-center gap-2 text-slate-500">❌ {{ app()->getLocale() == 'cs' ? 'Real-time počítání' : 'Real-time counting' }}</li>
                        <li class="flex items-center gap-2 text-slate-500">❌ {{ app()->getLocale() == 'cs' ? 'Push notifikace' : 'Push notifications' }}</li>
                    </ul>
                    <a href="{{ route('contact', ['interest' => 'MaponCulture Single']) }}" class="block text-center py-3 rounded-xl border-2 border-purple-500 text-purple-500 font-bold hover:bg-purple-500/10 transition-colors">{{ app()->getLocale() == 'cs' ? 'Objednat akci' : 'Order event' }}</a>
                </div>
                <!-- Festival -->
                ...
                <div class="bg-[#1e293b]/50 rounded-2xl border border-slate-800 p-8 flex flex-col shadow-sm">
                    ...
                    <p class="text-slate-400 text-sm mb-6">{{ app()->getLocale() == 'cs' ? 'Celoroční podpora agentur' : 'Year-round agency support' }}</p>
                    <div class="mb-6">
                        <span class="text-4xl font-black text-white">{{ app()->getLocale() == 'cs' ? 'Individuálně' : 'Individually' }}</span>
                    </div>
                    <ul class="space-y-4 mb-8 flex-grow text-sm text-slate-300">
                    ...
                    <a href="{{ route('contact', ['interest' => 'MaponCulture Pro']) }}" class="block text-center py-3 rounded-xl border-2 border-slate-700 text-white font-bold hover:bg-slate-700 transition-colors">{{ app()->getLocale() == 'cs' ? 'Poptat řešení' : 'Inquire solution' }}</a>
                </div>
            </div>
        </div>
